...
% Coefficients
alpha_W = (Gamma / dx) + rho * u / 2; % West coefficient
alpha_E = (Gamma / dx) - rho * u / 2; % East coefficient
...
for i = 1:N
    ...
    elseif i == N
        ...
        A(i, i) = alpha_W + alpha_E; % Central coefficient
...
% Plot results
figure;
plot(x, phi, 'bo--','LineWidth', 0.5 , 'MarkerFaceColor', 'b', 'DisplayName', 'Numerical (CDS)');
hold on;
plot(x, phi_analytical, 'r-','LineWidth', 1 , 'DisplayName', 'Analytical Solution');
xlabel('\it x (m)','Fontsize', 15, 'Interpreter','latex');
ylabel('\phi','Fontsize', 15);
set(gca, 'YDir', 'reverse'); % Invert the y-axis
ylim([20 160]); % Set y-axis limits
xlim([0 1]); % Ensure x-axis ranges from 0 to 1
legend('Location', 'SouthWest'); % Adjust legend position
grid on;
title('Analytical vs Numerical solution using CDS','Fontsize', 20 ,'Interpreter','latex');
